...
//source directory
config.src = 'site/assets/';

//destinations
config.dest = 'site/craft/html/assets/';
...
gulp.task('default', ['styles', 'scripts-individual', 'scripts-combined', 'inject-vendor', 'inject-scss']);
...
gulp.task('inject-vendor', function() {
    gulp.src('./site/craft/templates/_layouts/application.twig')
        .pipe(wiredep({
               'ignorePath': '../../html/',
							fileTypes: {
				        twig: {
				          replace: {
				            js: '<script src="{{ siteUrl }}{{filePath}}"></script>',
										css: '  <link rel="stylesheet" href="{{ siteUrl }}{{filePath}}" />'
				          }
				        }
				      }
           }))
        .pipe(gulp.dest('./site/craft/templates/_layouts'));
});
